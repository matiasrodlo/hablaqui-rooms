  ...
  const connect = useCallback(
    (token: string) => {
      Client.create(token)
        ...
        .catch(e => {
          console.error(e);
          onError(new Error('Hay un problema con nuestro proveedor de servicios. Intenta en unos minutos.'));
        });
    },
    [onError]
  );
  ...
  useEffect(() => {
    if (room && chatClient) {
      chatClient
        ...
        .catch(e => {
          console.error(e);
          onError(new Error('Tenemos problemas con el chat en este momento.'));
        });
    }
